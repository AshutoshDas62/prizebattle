...
function App(){
 ...
 const TournamentCard=({t})=><article className={'t-card '+t.color}><div className="game-art"><span className="game-logo">{t.game==='BGMI'?'BATTLEGROUNDS':'FREE FIRE'}</span><div className="smoke"></div><span className="fighter">♟</span><em>{t.tag}</em></div><div className="card-body"><div className="game-line"><span>{t.game}</span><small>{t.mode}</small></div><h3>{t.title}</h3><div className="details"><span>⌚ {t.time}</span><span>♙ {t.spots}</span></div><div className="card-foot"><div><small>Prize Pool</small><strong>{t.prize}</strong></div><button onClick={()=>join(t)} className={joined.includes(t.id)?'joined':''}>{joined.includes(t.id)?'✓ Joined':`Join ₹${t.fee}`}</button></div></div></article>;
 ...
 const Login=()=> <main className="inner"><div className="page-title"><p className="eyebrow">WELCOME BACK</p><h1>Log in to your arena.</h1></div><form className="empty" onSubmit={handleLogin} style={{display:'grid',gap:'14px',maxWidth:'420px',margin:'40px auto 0'}}><input id="login-name" defaultValue={userName} placeholder="Player name" style={{padding:'14px 16px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'16px'}} /><input id="login-password" type="password" placeholder="Password" style={{padding:'14px 16px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'16px'}} /><button type="submit" className="primary">Continue to dashboard</button><button type="button" className="secondary" onClick={()=>setPage('signup')}>Create an account</button></form></main>;
 ...
 return <><Nav/>{fetchError && <div className="toast">⚠ {fetchError}</div>}{content}{toast&&<div className="toast">✓ {toast}</div>}<footer><button className="brand"><b>PRIZE</b><i>BATTLE</i><span>✦</span></button><p>© 2026 PrizeBattle. Built for players who play to win.</p><div>Terms & Conditions &nbsp; Privacy &nbsp; Support</div></footer></>
}
