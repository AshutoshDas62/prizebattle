...
function App(){
 ...
 const TournamentCard=({t})=>{const image=t.game==='BGMI'?'/images/BGMI.jpg':'/images/free_fire.jpg';return <article className={'t-card '+t.color}><div className="game-art" style={{backgroundImage:`url(${image})`}}><span className="game-logo">{t.game==='BGMI'?'BATTLEGROUNDS':'FREE FIRE'}</span><em>{t.tag}</em></div><div className="card-body"><div className="game-line"><span>{t.game}</span><small>{t.mode}</small></div><h3>{t.title}</h3><div className="details"><span>⌚ {t.time}</span><span>♙ {t.spots}</span></div><div className="card-foot"><div><small>Prize Pool</small><strong>{t.prize}</strong></div><button onClick={()=>join(t)} className={joined.includes(t.id)?'joined':''}>{joined.includes(t.id)?'✓ Joined':`Join ₹${t.fee}`}</button></div></div></article>};
