...
function App(){
 ...
 const handleSignup=(e)=>{
  ...
  fetch('/api/signup', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ name, email, phone, password, confirmPassword }) })
    .then(async (res)=>{
      const data = await res.json();
      if(!res.ok || !data.success){
 ...
 const SignupPhoneField=()=>page==='signup'?<div className="signup-phone-field"><label htmlFor="signup-phone">Phone number for account recovery</label><input id="signup-phone" type="tel" placeholder="+919812345678" required /></div>:null;
 ...
 return <><Nav/>{fetchError && <div className="toast">⚠ {fetchError}</div>}{content}<SignupPhoneField/>{page==='login' && firebaseEnabled && <PhoneLogin onLogin={notify}/>} {toast&&<div className="toast">✓ {toast}</div>}<footer><button className="brand"><b>PRIZE</b><i>BATTLE</i><span>✦</span></button><p>© 2026 PrizeBattle. Built for players who play to win.</p><div>Terms & Conditions &nbsp; Privacy &nbsp; Support</div></footer></>
}
